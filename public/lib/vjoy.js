...
(function (window, Phaser) {
  ...
  var setDirection = function () {
    ...
    this.cursors.up = (deltaY < 0 && ((Math.abs(deltaY) / Math.abs(deltaX)) > .35));
    this.cursors.down = (deltaY > 0  && ((Math.abs(deltaY) / Math.abs(deltaX)) > .35));
    this.cursors.left = (deltaX < 0 && ((Math.abs(deltaX) / Math.abs(deltaY)) > .35));
    this.cursors.right = (deltaX > 0 && ((Math.abs(deltaX) / Math.abs(deltaY)) > .35));
